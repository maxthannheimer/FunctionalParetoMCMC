...
#Run MCMC Algorithm
println(param_start)
@time (result= MCMC(N_MCMC,observation_data,observation_x0,threshold, alpha, coord_fine,coord_coarse,param_start,row_x0,10,N_est_c,N_est_cond))


#mean of params with burn in period canceled out
[mean(result["beta"][N_burn_in+1:N_MCMC+1]),mean(result["c"][N_burn_in+1:N_MCMC+1])]
